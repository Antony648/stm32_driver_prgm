...
{//the dummy algorithm to perform diagnostics for power led
	printf("algorithm to performdiagnostics on powerled\n");
}
uint32_t PowerLed_effeciency(PowerLed_Type const*const obj)
{//the dummuy algorithm to compute effecincy for power led
	printf("algorithm to perform power effeciency on powerled\n");
	return 1;
}
									...
									LedVoltage_Type _voltage)
{
	static const struct LedVtable vtable={
		(void(*)(Led const *const obj))&PowerLed_diagnostics,
		(uint32_t(*)(Led const*const obj))&PowerLed_effeciency
	};
	Led_ctor(&pled_obj->super,_color,_state);
	pled_obj->current=_current;
	pled_obj->diameter=_diameter;
	pled_obj->voltage=_voltage;
	pled_obj->super.virt=&vtable;
